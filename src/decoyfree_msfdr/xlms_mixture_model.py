...
class MixtureModel(MixtureModelBase):
    ...
    def cdf(self, x):
        cdf_c = self.all_comps['C'].cdf(x)
        cdf_ic = self.all_comps['IC'].cdf(x)
        cdf_i1 = self.all_comps['I1'].cdf(x)
        return self.weights[0]['C'] * cdf_c + self.weights[0]['IC'] * cdf_ic + self.weights[0]['I1'] * cdf_i1
    ...
    def from_json(model_json, X):
        model = MixtureModel({k: 1 for k in ['C', 'IC', 'IC2', 'I1', 'I2']}, ic2_comp='IC2' in model_json)
        for k in ['C', 'IC', 'IC2', 'I1', 'I2']:
            param = model_json[k]
